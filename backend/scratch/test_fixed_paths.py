...
def calculate_walkable_distances_fixed(parsed_data: dict[str, Any]) -> dict[str, Any]:
...
dxf_data = calculate_walkable_distances_fixed(parse_dxf_file(dxf_file))
pdf_data = calculate_walkable_distances_fixed(parse_pdf_file(pdf_file, page_index=1))

print("\n--- FIXED DXF TRAVEL DISTANCES ---")
for r in dxf_data["rooms"]:
    print(f"  {r['name']:<25} | Travel Dist: {r['travel_distance_m']:>6.2f} m | Nearest Exit: {r['nearest_exit']}")

print("\n--- FIXED PDF TRAVEL DISTANCES ---")
for r in pdf_data["rooms"]:
    print(f"  {r['name']:<25} | Travel Dist: {r['travel_distance_m']:>6.2f} m | Nearest Exit: {r['nearest_exit']}")
